...
## Verdict
- **Request changes** if anything is wrong or unclear: leave specific, actionable
  comments (file + line + what to change and why). Any comment means the PR goes back
  to the Developer — do not approve.
- **Approve** only when the diff fully satisfies the standards and CI is green. State
  briefly what you verified.
